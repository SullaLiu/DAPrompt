...
def main(args, config):
    ...
    # )
    # # 假设model是你的gloss_free_model实例，peft_config是QLoRA配置
    # lora_model = get_peft_model(model, peft_config)
    # print(f"原模型参数: {sum(p.numel() for p in model.parameters()):,}")
    # print(f"QLoRA新增参数: {sum(p.numel() for p in lora_model.parameters() if p.requires_grad):,}")
    # return
    if args.finetune:
        print('***********************************')
