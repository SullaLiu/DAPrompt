...
# *user-defined
from models import  SLRCLIP_Prompt
...
def semantic_divergence_loss(image_features, text_features, epsilon=1e-8):
    """
    Computes the Semantic Divergence Loss.
    
    Args:
        image_features (torch.Tensor): L2-normalized image features of shape [N, D].
        text_features (torch.Tensor): L2-normalized text features of shape [N, D].
        epsilon (float): A small value to ensure numerical stability.
        
    Returns:
        torch.Tensor: The calculated Semantic Divergence Loss.
    """
    # --- Calculate loss for visual features ---
    # 1. Compute pairwise L2 distances between all image features. Resulting shape: [N, N]
    image_dist_matrix = torch.cdist(image_features, image_features, p=2)
    
    # 2. Apply the formula: log(1 + 1 / (epsilon + distance))
    # This value is high for small distances and low for large distances.
    image_loss_matrix = torch.log(1 + 1 / (epsilon + image_dist_matrix))
    
    # 3. Exclude the diagonal elements, as the formula sums over i != j.
    image_loss_matrix.fill_diagonal_(0)
    
    # 4. Sum the loss terms for all pairs.
    loss_v = image_loss_matrix.sum()

    # --- Calculate loss for text features (same logic) ---
    text_dist_matrix = torch.cdist(text_features, text_features, p=2)
    text_loss_matrix = torch.log(1 + 1 / (epsilon + text_dist_matrix))
    text_loss_matrix.fill_diagonal_(0)
    loss_t = text_loss_matrix.sum()
    
    # --- Combine and apply the scaling factor from the formula ---
    # The formula is (1/2) * sum over both modalities.
    total_sd_loss = 0.5 * (loss_v + loss_t)
    
    # Normalize by the number of pairs to make the loss independent of the batch size.
    N = image_features.size(0)
    if N > 1:
        total_sd_loss /= (N * N)
        
    return total_sd_loss
...
def main(args, config):
    ...
    model = SLRCLIP_Prompt(
        config=config
        )
    model = inplace_quantize_layers(model, len(train_dataloader) * 50, ptq = True if args.type == "PTQ" else False,
                              level = 'L', omse = True)
    model.to(device)
    print(model)
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
    ...
    for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss_sd = semantic_divergence_loss(logits_per_image, logits_per_text)
            total_loss = (loss_imgs + loss_texts)/2 + loss_sd/2.
